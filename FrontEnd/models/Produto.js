/* Produto — entidade com validações e serialização para o backend */
...
window.AGAPE.Models.Produto = (function () {
    ...
    function Produto(dados) {
        ...
        this._valorUni  = (dados.valorUni  !== undefined && dados.valorUni  !== null) ? parseFloat(dados.valorUni)    : null;
        this._qtdeAtual = (dados.qtdeAtual !== undefined && dados.qtdeAtual !== null) ? parseInt(dados.qtdeAtual, 10) : null;
    }
    ...
    Produto.prototype.validarValorUni = function () {
        return this._valorUni !== null && !isNaN(this._valorUni) && this._valorUni >= 0;
    };

    Produto.prototype.validarQtdeAtual = function () {
        return this._qtdeAtual !== null && !isNaN(this._qtdeAtual) && this._qtdeAtual >= 0;
    };
    ...
    Produto.prototype.validar = function () {
        ...
        if (!this.validarCategoria()) erros.push('Categoria de produto é obrigatória.');
        if (!this.validarValorUni())  erros.push('Valor unitário deve ser um número maior ou igual a zero.');
        if (!this.validarQtdeAtual()) erros.push('Quantidade deve ser um número inteiro maior ou igual a zero.');
        return erros;
    };

    // Inclui idProd apenas na edição (PUT); omite no cadastro (POST)
